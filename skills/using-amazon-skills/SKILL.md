...
| **Large** (1wk+, feature changing architecture or new service) | New service, new public API surface, new pattern adoption (e.g., move to cell-based), schema migration, multi-region rollout, security model change | Full flow: `/wb` (or 5CQ if customer is known) → `/design` (Design Doc + API Contract + Threat Model + Review) → `/spec` per slice → `/build` | Full flow artifacts | The Medium set plus `security-guardian` and `ops-bar-raiser` where the design and the rollout demand them | The Medium set | The Medium set | The Medium set |
| **New product** (months, novel customer value) | A new product, a new business line, a major rewrite | Full `/wb` cycle: `/listen` → `/define` → `/invent` → `/refine` → `/test-idea` → `/design` → ... | Full flow artifacts | The Large set plus `doc-bar-raiser` on the PR/FAQ | The Large set | The Large set | The Large set |
