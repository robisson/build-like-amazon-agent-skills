...
| Change size | Examples | Recommended path | Artifacts produced | Mandatory bar raisers | Deterministic verification that runs | Completion evidence required | Iteration budget | Flow events emitted |
|---|---|---|---|---|---|---|---|---|
| **Trivial** (< 2h, no customer impact) | Typo fix, dependency bump, comment cleanup, internal helper rename, dev-only logging tweak | Direct to code with Code Quality Bar from `incremental-implementation`. No spec, no Design Doc, no PR/FAQ. | Code + tests | None mandatory | None: there is no spec to check | None | Inline, no persisted report | None |
| **Small** (2h-1d, customer-facing but local) | Add a button, change copy, add a field to an existing API response (additive, backward-compatible), add an alarm, fix a known bug with clear root cause | Direct to code OR a single lightweight spec without a separate Design Doc. Apply TDD + operational-code + Code Quality Bar. | Code + tests, optionally `.bla/specs/<slice>/tasks.md` | None mandatory | None | None | Inline | None |
| **Medium** (1d-1wk, feature in existing product) | New endpoint with new logic, new screen with new interactions, integrating a new external dependency, adding a feature flag and rolling out a new behavior | `/spec` (anchored on existing code or a reverse-engineered design — see `/onboard`) → `/build`. No `/wb`; the customer is already known. Mini design section if architectural decisions arise. | Full spec trio + code + tests | `implementation-verifier`, dispatched author-isolated; `design-bar-raiser` when a design exists | `bla-check tasks --wave <n>` at each wave close and `bla-check tasks` at spec close: markers, the `writes` intersection per wave, and the `git diff` unclaimed-path comparison | `.bla/specs/<slice>/.reports/<task-id>.md` per `[x]` task | Persisted report with finding IDs and a finding lifecycle | `.bla/metrics.jsonl`, the six events per `docs/flow-metrics.md` |
| **Large** (1wk+, feature changing architecture or new service) | New service, new public API surface, new pattern adoption (e.g., move to cell-based), schema migration, multi-region rollout, security model change | Full flow: `/wb` (or 5CQ if customer is known) → `/design` (Design Doc + API Contract + Threat Model + Review) → `/spec` per slice → `/build` | Full flow artifacts | The Medium set plus `security-guardian` and `ops-bar-raiser` where the design and the rollout demand them | The Medium set | The Medium set | The Medium set | The Medium set |
| **New product** (months, novel customer value) | A new product, a new business line, a major rewrite | Full `/wb` cycle: `/listen` → `/define` → `/invent` → `/refine` → `/test-idea` → `/design` → ... | Full flow artifacts | The Large set plus `doc-bar-raiser` on the PR/FAQ | The Large set | The Large set | The Large set | The Large set |

*Mandatory bar raisers* answers one question and only that one: which bar raisers a level **requires**. It never holds a conditional. Anything a level merely permits belongs here instead: at **Small**, `code-review-bar-raiser` is the one bar raiser worth invoking on request — it is not required, and the level is not held open waiting for it. Simplicity (`AGENTS.md` §4) sits outside the column at every level because it is always active and is never dispatched.

The five right-hand columns are what a level actually costs, and they are read **monotonically**: a case may lower ceremony *within* its level — a Medium change with no architectural decision needs no mini design section — but it may never raise it above what its level declares. Without that direction the ladder stops classifying and becomes a per-case negotiation about its own ceremony.

Two demands inside those columns are conditional, not universal, and the difference is the door. **Alternatives**: a one-way door requires at least two real alternatives, considered and recorded with the reason the chosen one won; a two-way door requires an alternative only when a plausible one exists, and *"there was only one plausible approach"* is a valid answer **if it is recorded with the why**. An invented second option is worse than an honest absence — it makes the record lie about how the decision was made. **Scale**: a 10x or 100x projection is derived from a projection the requirements declare, never from a multiplier the agent picks; with no declared projection the right output is a `QUESTION` asking for one (`AGENTS.md` → *One Severity Scale and One Verdict Scale*), which does not block by itself but prevents a clean APPROVED while the answer could change scope, contract or risk.
...
- **Repository state** — is this a project with existing artifacts (`.bla/design/`, `.bla/working-backwards/`) or an unstructured project? Affects which path is *available*, not which is *appropriate*.
...
2. There is a mismatch between the command invoked and the repo state (e.g., `/build` invoked but no `.bla/specs/` exists — see Step 0 in the command files)
...
- If the user invoked `/build` and `.bla/specs/` exists → execute as normal. Skip this rule.
- If the project has significant code/IaC/CI but **no BLA artifacts** (`.bla/design/`, `.bla/working-backwards/`):
...
If `/onboard` was previously run (artifacts in `.bla/design/<service>/` with the **REVERSE-ENGINEERED** banner), do not re-run unless the project has drifted significantly. Use those artifacts as the anchor exactly like canonical Design Doc artifacts.
