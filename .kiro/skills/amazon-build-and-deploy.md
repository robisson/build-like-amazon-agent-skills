...
4. **`/build` is autonomous to feature completion.** No gates between specs, waves, or PASSED reviews. Stops only for hard blockers, FAILED review, or explicit user pause request.
5. **Implementation memory** — Internally use `.kiro/skills/amazon/implementation-memory/SKILL.md` and `.bla/implementation-memory.md` during build. Multi-signal matching (Tags, File patterns, `Applies when`). Semi-auto trigger on PASSED WITH FIXES NEEDED (self-reflection → candidates → Accept/Reject/Edit). Nudge after 3 builds without update. Multi-source capture from `/review` and `/learn`. Staleness: flag rules unused >90 days. Do not expose as a user command.
