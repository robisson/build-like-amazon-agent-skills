...
1. For implementation: Read `.kiro/skills/amazon/spec-driven-implementation/SKILL.md` first, then `.kiro/skills/amazon/incremental-implementation/SKILL.md`. During `/build`, automatically read `.kiro/skills/amazon/implementation-memory/SKILL.md` after the current spec/tasks are known, apply only matching memory guardrails, prompt the user to test/debug after implementation review, and update memory only after user validation/request when a durable learning meets the admission bar. This is an internal build mechanism, not a user-facing command.
...
5. **Implementation memory** — Internally use `.kiro/skills/amazon/implementation-memory/SKILL.md` and `docs/implementation-memory.md` during build to apply compact lessons, prompt user validation after implementation review, and update memory only after validation/request. Do not expose this as a separate user command.
