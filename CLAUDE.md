...
- **During /build**: Execute autonomously **end-to-end across all specs**. NO approval gates between tasks, between waves, between specs, or after the Post-Implementation Review when verdict is PASSED / PASSED WITH FIXES NEEDED. Run all specs in `.bla/specs/` to terminal state in a single invocation. Stop only for: a hard blocker requiring user input, a FAILED review verdict, an unrecoverable green-build failure, or an explicit user request to pause (e.g., *"build spec by spec, ask me before each"*). See `.claude/commands/build.md` for the four valid stop reasons and the end-of-build summary format.
...
| `.claude/commands/` | 14 slash command definitions — **the files you edit.** `.gemini/commands/` and `.kiro/commands/` hold the same 14 as hand-synced mirrors, differing only by a banner and a path-resolution note; CI's `parity` step fails the build if one drifts. There is no generator and no separate canonical directory. |
| `docs/` | Getting started, skill anatomy, philosophy, [`docs/artifact-catalog.md`](docs/artifact-catalog.md) — the index of every artifact the flow produces: canonical path, mandatory or optional, template, producing command and consumers — and [`docs/flow-metrics.md`](docs/flow-metrics.md), the six flow events, their fields and their single owners. Both are reference documents, not skills. |
| `tools/` | `bla-check` — the deterministic document checker (task markers, task graph, relative links, flow-metric series) and its self-test in `tools/tests/`. Maintainer-side: run `python3 tools/tests/run.py`, `python3 tools/bla-check links .` and, if the project keeps a flow-metric series, `python3 tools/bla-check metrics <path>` before opening a PR. |
| `.github/workflows/` | `check.yml` — the CI job that re-runs the document checks on the server: README counts, skill and agent frontmatter, severity/verdict label integrity, cross-references, `bla-check links`, the self-test, `markdownlint`, `version-agreement` (`VERSION`, `.claude-plugin/plugin.json` and the first `CHANGELOG.md` heading must name the same version), `parity` (editing `.claude/commands/<name>.md` without copying the change into `.gemini/commands/` and `.kiro/commands/` fails the build) and a secret scan. Every failure is one `FALHA [rule-name]` line. |
