...
6. **`/build` is autonomous to feature completion.** Approval gates happened in `/design`. `/build` runs all specs end-to-end without asking between specs, waves, or PASSED reviews. Stops only for hard blockers, FAILED review, unrecoverable green-build failure, or explicit user pause request.
