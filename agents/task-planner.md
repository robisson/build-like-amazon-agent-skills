...
- **Reads:** `.bla/specs/<slice-name>/design.md` (components, interfaces, data models) and `.bla/specs/<slice-name>/requirements.md` (for traceability); `skills/spec-driven-implementation/templates/tasks-template.md`.
- **Writes (exactly one file):** `.bla/specs/<slice-name>/tasks.md` — including the Dependency Graph JSON with waves at the bottom.
- **Must not touch:** `.bla/specs/<slice-name>/requirements.md` and `.bla/specs/<slice-name>/design.md`. A task with no requirement to trace to means the spec is incomplete: send it back to `/spec` Step 1 and let the user approve the missing requirement. Inventing it inside `tasks.md` is how gold-plating enters a spec that was already approved.
