...
CASES = [
    ...
    # `metrics` — the six flow-metric events of docs/flow-metrics.md. One case per rule,
    # plus the two cases that pin the opt-in contract: an absent file and an incomplete
    # pair are both AVISO and exit 0, because measurement must never block a phase.
    ("metrics-clean",
     ["metrics", os.path.join(FIXTURES, "metrics-clean", "bla-metrics.jsonl")], 0,
     ["cycle time per (phase, spec)", "rework per phase",
      "design", "spec=checkout-api", "level=medium", "source=/build"],
     ["FALHA", "AVISO", "n/a"], None),
    ("metrics-file-absent",
     ["metrics", os.path.join(FIXTURES, "metrics-clean", "does-not-exist.jsonl")], 0,
     ["AVISO [metrics-file-absent]"], ["FALHA"], None),
    ("metrics-unpaired",
     ["metrics", os.path.join(FIXTURES, "metrics-unpaired", "bla-metrics.jsonl")], 0,
     ["AVISO [metrics-unpaired]", "no phase_completed", "n/a"], ["FALHA"], None),
    ("metrics-out-of-order",
     ["metrics", os.path.join(FIXTURES, "metrics-out-of-order", "bla-metrics.jsonl")], 1,
     ["FALHA [metrics-out-of-order]", "aggregation skipped"],
     ["cycle time per", "FALHA [metrics-malformed]"], None),
    ("metrics-unknown-event",
     ["metrics", os.path.join(FIXTURES, "metrics-unknown-event", "bla-metrics.jsonl")], 1,
     ["FALHA [metrics-unknown-event]", "documents_produced"],
     ["FALHA [metrics-malformed]"], None),
    ("metrics-duplicate-start",
     ["metrics", os.path.join(FIXTURES, "metrics-duplicate-start", "bla-metrics.jsonl")], 1,
     ["FALHA [metrics-duplicate-phase-start]", "the first, at line 1, is the one preserved"],
     ["FALHA [metrics-out-of-order]"], None),
    ("metrics-malformed",
     ["metrics", os.path.join(FIXTURES, "metrics-malformed", "bla-metrics.jsonl")], 1,
     ["FALHA [metrics-malformed]", "missing or empty required field(s): phase",
      "not a JSON object but a list", "not valid JSON"],
     ["FALHA [metrics-unknown-event]"], None),
]
